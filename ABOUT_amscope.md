...
`amscope` is a small Python audio microscope. It takes an audio file and looks at a few basic musical and acoustic features, th then saves images that shows what is happening inside the sound.

This project is meant as both a learning tool and a practical experiment. The first goal is not to build a perfect music-analysis system, but to understand the basic flow of loading audio, analyzing it, and creating useful visual output. It is the first step in full structural analysis of a recorded piece of music. 

## What happens when the script runs

Let's say you have an audio file called test.wav (other formats possible, but .wav is cleanest for output)
When you run:
...
The script uses `librosa` to load the audio file.
